...
# Major change: 
# Change go cc to difficial expression go
# Still too much edges, not good visulization
...
ah %>%
  ...
  subset(grepl("Homo sapiens", species))
ensDb <- ah[["AH75011"]] #This is ensembl 98

# Get the mapping between Entrez & ensembl
ens2entrez <- genes(ensDb) %>% # Get all genes encoded on ensDb
...
topTable <- read_csv("data/alpha.csv") %>%
  inner_join(ens2entrez, by = c("ID" = "gene_id")) 

# all_go is all go gene sets
all_go <- msigdbr(species = "Homo sapiens", category = "C5") %>% 
...
jiq <- ji$weight %>% quantile(probs=seq(0, 1, by=0.05))
ji$weight %>% .[.>0] %>% quantile(probs=seq(0, 1, by=0.05))

# Histogram
hist_ji <- ji$weight %>%
  hist(breaks = 50, main = paste("Distribution of DE gene sets Jaccard index"), xlim = c(0,1), ylim = c(1, 1500), xlab='Jaccard index', ylab='Frequency')
# 80% of value are in red, the rest in green
hist_ji_cut <- cut(hist_ji$breaks, c(-Inf, jiq[19], Inf))
plot(hist_ji, col=c("#ff9182", "#82ffb0")[hist_ji_cut], xlab = "Jaccard index", main = "")
...
plot((1:n - 1)/(n - 1), sort(x), type="l",
     xlab = "Jaccard index fraction",
     ylab = "Quantile")
# barplot
boxplot(ji$weight, ylab = "Jaccard index")
...
.html(
  ...
  + '<br>' +
  'Potential community name :  ' + d.nodesize
  + '<br>' +
  'Keywords :  ' + d.nodesize
  + '<br>' +
  'Top 5 genes :  ' + d.nodesize
)
.style('right', '50px')
.style('bottom', '50px')
...
gode_nw <- forceNetwork(Links = match,
                        ...
                        linkDistance = 150,
                        Value = "value",
                        linkWidth = JS("function(d) { return d.value*30; }"),
                        #linkWidth = JS("d.value*100"),
                        width = 500,
                        height = 500,
                        opacity = 0.9,
                        fontSize = 16,
...
cc_nw$x$nodes$gs_name <- G$nodes$gs_name
gode_nw$x$nodes$com_name <- 
